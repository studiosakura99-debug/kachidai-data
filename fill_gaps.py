#!/usr/bin/env python3
# 全43店舗の「最新リポート」を確実にCSVへ入れる（欠落日を埋める）
import re, csv, os, time, html as H, urllib.request, urllib.parse, datetime
...
DR=re.compile(r'([0-9]{1,2})/([0-9]{1,2})\([^)]*\)')
...
def parse(page,url):
    ...
    ds="%04d-%02d-%02d"%(yr,mo,da); hall=title[dm.end():].strip()
    out=[]
...
print("既存 %d行 / %d店舗 / 既知リポート%d件"%(len(rows),len({r['hall'] for r in rows}),len(have)))
added=0; results=[]
for i,name in enumerate(stores):
    try: body=fetch(ROOT+"/?s="+urllib.parse.quote(name))
    except Exception as e:
        results.append((name,"検索失敗",0)); print("search fail",name,e); continue
    best=[]
    for m in ANCHOR.finditer(body):
        t=TD.match(H.unescape(m.group(2)).strip())
        if t: best.append((dk(t.group(1)),t.group(2).strip(),m.group(1)))
    best.sort(reverse=True)
    got=0; newest="-"
    for d,store,rid_ in best[:3]:
        ...
        if g!="宮城県":
            print("  SKIP %s (%s)"%(store,g or "無し")); have.add(rid_); time.sleep(IV); continue
        rs=parse(page,"%s/%s/?kishu=all"%(ROOT,rid_))
        if rs:
            rows.extend(dict(zip(HEADER,[str(x) if x is not None else '' for x in r])) for r in rs)
            have.add(rid_); got+=1; added+=len(rs); newest=rs[0][0]
        time.sleep(IV)
    results.append((name,newest,got))
    if got: print("  +%s %s (%d件)"%(name,newest,got))
    time.sleep(IV)
# 書き出し（重複排除・90日）
...
import collections
days=collections.Counter(r['business_date'] for r in ded)
newest={}
for r in ded:
    h=r['hall']; d=r['business_date']
    if h not in newest or d>newest[h]: newest[h]=d
latest=max(days)
gap=sorted(h for h in {r['hall'] for r in ded} if newest.get(h)!=latest)
print("="*50)
...
for h in gap: print("   -",h,newest.get(h))
